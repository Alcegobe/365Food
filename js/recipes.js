...
export function emptyFilters() {
  return { category: 'all', query: '', maxPoints: null, maxMinutes: null, tags: [] };
}
 ...
 * points max, temps max, tags (tous requis).
 */
...
/**
 * Charge les deux fichiers de données. `inline` permet d'injecter les données
 * (tests, version « un seul fichier ») sans réseau.
 */
export async function loadCatalog({ base = 'data/', inline = globalThis.__365FOOD_DATA__ } = {}) {
  let recipesDb;
  let ingredientsDb;
  if (inline) {
  ...
  return { recipes, ingredientIndex, nutritionById, aisles: ingredientsDb.aisles ?? {} };
}
